...
- Páginas (entregables): `src/pages/Contacto.tsx`, `src/pages/Nosotros.tsx`, `src/pages/Auditoria.tsx` (Servicios · Auditoría energética) y `src/pages/Ingenieria.tsx` (Servicios · Ingeniería). El catálogo Energía/Agua compartido vive en `src/pages/sections/SolutionsCatalog.tsx`. Datos compartidos (contacto, navegación, footer) en `src/pages/site.ts`. Fotos en `public/images/`, logos en `public/logo.png` / `public/logo-inverse.png`
...
- Vistas por hash: `#servicios`, `#ingenieria`, `#nosotros`, `#contacto` (por defecto), `#` (design system), `#page` (demo). Los enlaces del navbar y footer cambian de página.
